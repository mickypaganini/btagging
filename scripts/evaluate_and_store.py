...
def main(weights, filename, treename = 'bTag_AntiKt2PV0TrackJets', picklename = './val.pkl'):
    '''
    ...
    -----
        ...
        picklename: (optional) name of the output pickle to store new mv2 values
    Returns:
    --------
        status
    Raises:
...
if __name__ == '__main__':
    ...
    if (args.tree != None):
        sys.exit( main(args.weights, args.filename, args.tree) )
        if (args.picklename != None):
            sys.exit( main(args.weights, args.filename, args.tree, arg.picklename) )
    else:
        if (args.picklename != None):
            sys.exit( main(args.weights, args.filename, picklename = args.picklename) )
        else:
            sys.exit( main(args.weights, args.filename) )
